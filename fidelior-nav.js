/* ==========================================================================
   Fidelior Nav  v1.0  —  Tab-Navigation & Echtzeit-Dashboard
   ==========================================================================
   ZWECK: Tab-Navigation + vollständiges Dashboard als Startbildschirm.
   TABS:  [📊 Dashboard] [📥 Ablage] [📁 Archiv] [✅ Aufgaben] [⚙ Admin]
...
function computeStats({ activity, tasks, indexDocs }) {
  ...
  for (const t of openTasks) if (t.objectCode && byObj[t.objectCode]) byObj[t.objectCode].openTasks++;
...
const fmtShort = iso => { try { return new Date(iso).toLocaleDateString('de-DE',{day:'2-digit',month:'2-digit'}); } catch { return '—'; }};
...
function buildNav() {
  ...
  const spacer = document.createElement('div'); spacer.className = 'fdl-nav-spacer';
...
function showDash() { buildDash(); document.getElementById('fdl-dash-panel').classList.add('visible'); renderDash(); }
function hideDash() { document.getElementById('fdl-dash-panel')?.classList.remove('visible'); }
...
async function renderDash() {
  ...
  catch(e) { root.innerHTML = `<p style="color:#B91C1C;padding:20px">Fehler: ${e.message}</p>`; return; }
  ...
  const stats = `<div class="fdl-stats">
    ${sc(s.total, 'Dokumente gesamt', 'mu', 'Gesamtarchiv', "window.__fdlNav.goArchiv()")}
    ...
    ${sc(s.openTasks, 'Offene Aufgaben',
      ...
      s.overdueTasks>0?`⚠ ${s.overdueTasks} überfällig`:s.openTasks>0?'Offen':'✓ Alles erledigt',
      "window.__fdlNav.goTasks()")}
    ${sc(s.monthCount, 'Diesen Monat', 'mu', now.toLocaleDateString('de-DE',{month:'long'}), "window.__fdlNav.goArchiv()")}
  ...
  const liegCards = objList.length ? objList.map(o => {
    ...
    return `<div class="fdl-lc" onclick="window.__fdlNav.goArchivObj('${o.code}')" title="${o.name}">
      ...
      <div class="fdl-lc-pills">
        ...
        ${o.openTasks>0?`<span class="fdl-pill t">✅ ${o.openTasks}</span>`:''}
      </div>
  ...
  root.innerHTML = `
    <div class="fdl-dw">
      <div>
        <div class="fdl-dw-t">Guten Tag · ${dayStr}</div>
        <div class="fdl-dw-s">${dateStr}</div>
      </div>
    ...
    <div class="fdl-dg">
      <div>
        <div class="fdl-lh">Liegenschaften (${objList.length})</div>
...
async function buildInbox() {
  ...
  } catch { return '<div class="fdl-empty" style="font-size:12px;padding:14px">Inbox nicht verfügbar</div>'; }
}
...
window.__fdlNav = {
  ...
  refreshDash() { if (_tab === 'dash') renderDash(); },
};
...
function watchArchiv() {
  ...
  if (!av3) { setTimeout(watchArchiv, 600); return; }
...
function attachShortcuts() {
  document.addEventListener('keydown', e => {
    ...
    if (e.key==='d'||e.key==='D') { e.stopImmediatePropagation(); activateTab('dash'); }
  }, true);
}
...
function init() {
  ...
  console.info('[FideliorNav v1.0] bereit — D:Dashboard 2:Ablage 3:Archiv 4:Aufgaben');
}
