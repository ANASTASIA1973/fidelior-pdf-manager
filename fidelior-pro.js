/* ==========================================================================
   ...
   ==========================================================================
   CHANGES v2.2:
   - Dashboard nutzt primär echte Archivdaten aus Scopevisio
   - Sidebar-Counts nutzen primär echte Archivdaten
   - IndexedDB bleibt nur Fallback
   - Keine Änderung an Ablage-Logik oder Scopevisio-Struktur
   ========================================================================== */
...
/* ══════════════════════════════════════════════════════════════════════════
   ARCHIV HELPERS — echte Scopevisio-Daten
   ══════════════════════════════════════════════════════════════════════════ */

let _objectsMap = null;
const ARCHIVE_CACHE_MS = 45000;
let _archiveCache = { ts: 0, docs: [] };
...
async function loadObjectsConfig() {
  ...
  try {
    ...
    console.warn('[FideliorPro] objects.json:', e);
  }
  return _objectsMap;
}
...
function deriveSavedAt(metaDate, modified) {
  const iso = toISOFromFilenameDate(metaDate);
  ...
  for (const obj of objList) {
    ...
    for (const { segs, label } of roots) {
      ...
      for (const f of batch) {
        ...
        const year = extractYear(f.pathSegs, f.modified);

        all.push({
          fileName: f.name,
          objectCode: obj.code,
          objectName: getScopeName(obj.code),
          docType: folderTypeToDocType(label),
          amount: parseAmount(meta?.betrag),
          amountRaw: meta?.betrag || '',
          sender: meta?.absender || '',
          senderNorm: (meta?.absender || '').toLowerCase(),
          invoiceDate: toISOFromFilenameDate(meta?.datum),
          savedAt: deriveSavedAt(meta?.datum, f.modified),
...
function computeStats(data) {
  const { activity, tasks, indexDocs, archiveDocs } = data;
  ...
  const nonBranchObjectCount = Object.values(byObj).filter(o => !BRANCH_CODES.has(o.code) && o.count > 0).length;
  const catCounts = {
    Objekte: nonBranchObjectCount,
    Fidelior: byObj.FIDELIOR?.count || 0,
    Privat: byObj.PRIVAT?.count || 0,
    'ARNDT & CIE': byObj.ARNDTCIE?.count || 0,
  };

  return {
    total: docs.length,
    wkCount: wkDocs.length,
    moCount: moDocs.length,
    openCount: open.length,
    overdueCount: overdue.length,
    moAmt,
    byObj,
    recent,
    openTasks: open.slice(0, 8),
    yr,
    catCounts,
...
function buildTopbar() {
  ...
  document.getElementById('fdl-tb-refresh').addEventListener('click', () => { if (_view === 'dash') { invalidateArchiveCache(); renderDash(); } });
}
...
function buildMain() {
  ...
  dashV.id = 'fdl-view-dash';
  dashV.className = 'fdl-view';
  const archV = document.createElement('div');
  archV.id = 'fdl-view-archive';
  archV.className = 'fdl-view';
  content.appendChild(dashV);
  content.appendChild(archV);
...
async function renderDash() {
  ...
  try {
    ...
    root.innerHTML = `<p style="color:var(--pro-red);padding:20px">Fehler: ${e.message}</p>`;
    return;
  }
  ...
  const kpis = `<div class="fdl-kpis">
    <div class="fdl-kpi" onclick="window.__fdlPro.goArchiv()">
      ...
      <div class="fdl-kpi-trend neu">${s.source === 'archive' ? 'Gesamtarchiv' : 'Index/Fallback'}</div>
    </div>
...
function updateBadges(s) {
  ...
  if (tb) {
    if (s.openCount > 0) {
      tb.textContent = s.openCount;
      tb.style.display = 'inline-flex';
      tb.className = 'fdl-sb-badge' + (s.overdueCount > 0 ? ' amber' : '');
    } else {
...
async function updateSidebarCounts() {
  let docs = [];
  try {
    docs = await getArchiveDocsCached();
  } catch {}

  if (!docs.length) {
    docs = await idbGetAll('fidelior_index_v1', 'documents').catch(() => []);
    if (!docs.length) docs = await idbGetAll('fidelior_addon_v1', 'activity').catch(() => []);
  }

  const counts = {};
  for (const d of docs) {
    if (d.objectCode) counts[d.objectCode] = (counts[d.objectCode] || 0) + 1;
  }

  document.querySelectorAll('[id^="fdl-sbcnt-"]').forEach(el => { el.textContent = ''; });
